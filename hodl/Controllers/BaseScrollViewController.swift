...
extension UIViewController {
    ...
    func showToast(message : String, font: UIFont) -> UILabel? {
        let toastHeight = CGFloat(50)
        let toastWidth  = self.view.frame.size.width/2
        let xPos = self.view.frame.size.width/2 - toastWidth/2
        let yPos = self.view.frame.size.height/2 - toastHeight/2
        let toastLabel = UILabel(frame: CGRect(x: xPos, y: yPos, width: toastWidth, height: toastHeight))
        ...
        UIView.animate(withDuration: 10.0, delay: 0.1, options: [.curveEaseOut, .repeat], animations: {
